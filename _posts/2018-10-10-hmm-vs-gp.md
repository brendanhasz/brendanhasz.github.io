...
Also, in addition to being pulled towards the value of the other datapoint, notice that the probability distributions are, in addition, pulled towards \\( 0 \\).  This is because the gaussian process assumes that the mean of all the data is \\( 0 \\) (or at least ours does, because we're using a mean function of \\( 0 \\)).
...
functions which don't).  we'll use the "squared exponential" kernel (aka the 
