...
After a while (could be up to a business day or two), your service limit will be increased.  Now you can run the instance(s) for which you requested the service increase!
...
Now you need to copy your data from your local machine (or wherever) to your EC2 instance.  There are a few different ways you can do this: load external data via scp, load data from S3, upload data from your local computer, and more.
...
However, instead of uploading data from your computer and then copying it to the container, it's usually easier and faster to "expose" and "publish" a port on the container.  To do so, when running the image, you'll need to tell docker to allow incoming connections through a specific port, and to pipe connections from the host's (your EC2 instance) port to the container port.  You can do this by running (when you start your container):

```bash
docker run --expose 22 -p 55555:22 <image-name>
```

This allows connections on port 22 (the default for scp and ssh), and causes connections to port 55555 on your EC2 server to be piped to port 22 of the container.  The host port doesn't have to be 55555, it can be any [unassigned port](https://en.wikipedia.org/wiki/List_of_TCP_and_UDP_port_numbers) (49152 - 65535).

Then, to copy data to your container directly from your local machine, 

```bash
scp -P 55555 -i <key-pair-name>.pem <filename> root@<publicDNS>:<filename-on-container>
```
...
EC2 instances can be left continuously running, which is what you'll want to do if you're running a server or a website or a service.  However, if you just ran a batch of analysis code, you'll want to download the results.  After your analysis code has finished running and produced output data, you can either copy the data from your container to a server using `scp`,  write the data to an S3 bucket using `boto3` or use some other method.
